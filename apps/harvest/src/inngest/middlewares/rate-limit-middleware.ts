...
const harvestRateLimitError = z.object({
  errors: z.array(
    z.object({
      extensions: z.object({
        ...
        return {
          transformOutput: async (ctx) => {
            ...
            try {
              ...
              const isRateLimitError = harvestRateLimitError.safeParse(response).success;
              if (!isRateLimitError) {
                return;
              }
            } catch (_error) {
            ...
            // https://developers.harvest.app/docs/graphql/working-with-the-graphql-api/rate-limiting
