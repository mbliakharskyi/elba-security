...
describe('unauthorized middleware', () => {
  ...
  test('should not transform the output when the error is not about Pipedrive authorization', async () => {
    const send = vi.fn().mockResolvedValue(undefined);
    await expect(
      unauthorizedMiddleware
        // @ts-expect-error -- this is a mock
  ...
  test('should transform the output error to NonRetriableError and remove the organisation when the error is about Pipedrive authorization', async () => {
    const unauthorizedError = new PipedriveError('foo bar', {
