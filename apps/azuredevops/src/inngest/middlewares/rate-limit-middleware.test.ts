...
describe('rate-limit middleware', () => {
  ...
  test('should not transform the output when the error is not about Azuredevops rate limit', () => {
    expect(
      rateLimitMiddleware
        .init()
        // @ts-expect-error -- this is a mock
  ...
  test('should transform the output error to RetryAfterError when the error is about Azuredevops rate limit', () => {
    const rateLimitError = new AzuredevopsError('foo bar', {
      // @ts-expect-error this is a mock
      response: {
        status: 429,
