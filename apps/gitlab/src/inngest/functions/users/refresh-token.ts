...
export const refreshSaaSToken = inngest.createFunction(
  {
    id: 'gitlab-refresh-gitlab-token',
    concurrency: {
    ...
    cancelOn: [
      {
        event: `gitlab/gitlab.elba_app.uninstalled`,
        match: 'data.organisationId',
      },
      {
        event: `gitlab/gitlab.elba_app.installed`,
        match: 'data.organisationId',
      },
    ],
    retries: env.TOKEN_REFRESH_MAX_RETRY,
  },
  { event: 'gitlab/gitlab.token.refresh.requested' },
  async ({ event, step }) => {
