...
describe('auth connector', () => {
  describe('getToken', () => {
    beforeEach(() => {
      server.use(
        http.post(`${env.SALESLOFT_APP_INSTALL_URL}/token`, async ({ request }) => {
    ...
    test('should throw when the code is invalid', async () => {
      await expect(getToken('wrong-code')).rejects.toBeInstanceOf(SalesloftError);
    });
  });
  ...
  describe('getRefreshToken', () => {
    ...
    test('should throw when the refreshToken is invalid', async () => {
      await expect(getToken('wrong-refreshtoken')).rejects.toBeInstanceOf(SalesloftError);
    });
  });
});
