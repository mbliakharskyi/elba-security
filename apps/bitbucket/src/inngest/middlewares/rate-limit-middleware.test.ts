...
describe('rate-limit middleware', () => {
  ...
  test('should not transform the output when the error is not about Bitbucket rate limit', () => {
    expect(
      rateLimitMiddleware
        .init()
        // @ts-expect-error -- this is a mock
  ...
  test('should transform the output error to RetryAfterError when the error is about Bitbucket rate limit', () => {
    const rateLimitError = new BitbucketError('foo bar', {
      // @ts-expect-error this is a mock
      response: {
        status: 429,
