...
describe('auth connector', () => {
  describe('getToken', () => {
    beforeEach(() => {
      server.use(
        http.post(`${env.INTERCOM_APP_INSTALL_URL}/oauth_token`, async ({ request }) => {
  ...
  describe('getRefreshToken', () => {
    ...
    test('should throw when the refreshToken is invalid', async () => {
      await expect(getToken('wrong-refreshtoken')).rejects.toBeInstanceOf(IntercomError);
    });
  });
  ...
  describe('getWorkspaceIds', () => {
    beforeEach(() => {
      server.use(
        http.get(`${env.INTERCOM_API_BASE_URL}/workspaces`, ({ request }) => {
    ...
    test('should throw when the accessToken is invalid', async () => {
      await expect(getWorkspaceIds(invalidToken)).rejects.toBeInstanceOf(IntercomError);
    });
  });
});
