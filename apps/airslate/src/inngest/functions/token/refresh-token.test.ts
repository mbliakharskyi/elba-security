...
import * as authConnector from '@/connectors/airslate/auth';
...
const setup = createInngestFunctionMock(refreshToken, 'airslate/token.refresh.requested');
...
describe('refresh-token', () => {
  ...
  test('should update encrypted tokens and schedule the next refresh', async () => {
    ...
    if (!updatedOrganisation) {
      throw new AirslateError(`Organisation with ID ${organisation.id} not found.`);
    }
